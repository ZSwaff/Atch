...
package com.auriferous.atch.UiElements.SwipeRefreshLayoutBottom;
...
public class SwipeRefreshLayoutBottom extends ViewGroup {
    ...
    private final Animation mAnimateToCorrectPosition = new Animation() {
        ...
        public void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            if (!mUsingCustomStart) {
                endTarget = getMeasuredHeight() - (int) (mSpinnerFinalOffset); // toxxxdo
            } else {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!mUsingCustomStart && !mOriginalOffsetCalculated) {
            ...
            mCurrentTargetOffsetTop = mOriginalOffsetTop = getMeasuredHeight() - mCircleView.getMeasuredHeight();  // toxxxdo
        }
        mCircleViewIndex = -1;
    ...
    public boolean canChildScrollDown() {   // toxxxdo
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (mTarget instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) mTarget;
                try {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollDown() || mRefreshing) {  // toxxxdo
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                final float yDiff = mInitialMotionY - y;   // toxxxdo
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || canChildScrollDown()) {  // toxxxdo
            // Fail fast if we're not in a state where a swipe is possible
            return false;
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                final float overscrollTop = (mInitialMotionY - y) * DRAG_RATE;  // toxxxdo
                if (mIsBeingDragged) {
